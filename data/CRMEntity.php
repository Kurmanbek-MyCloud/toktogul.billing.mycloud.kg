...
//Added by Richie
	...
	function trash($module, $id) {
		...
		if ($module == 'Invoice' || $module == 'SPPayments' || $module == 'Penalty') {
			if ($module == 'Invoice') {
				$flat_id = $adb->run_query_field("SELECT cf_1265 FROM vtiger_invoicecf WHERE invoiceid = $id");
			} elseif ($module == 'SPPayments') {
				$flat_id = $adb->run_query_field("SELECT cf_1416 FROM sp_paymentscf WHERE payid = $id");
			} elseif ($module == 'Penalty') {
				$flat_id = $adb->run_query_field("SELECT vi.cf_1265 FROM vtiger_penalty vp 
