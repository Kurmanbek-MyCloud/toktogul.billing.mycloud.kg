...
//since support for multiple calendar views for events is enabled
			...
			submitHandler: function (form) {
				...
				app.request.post({data: formData}).then(function (err, data) {
					...
					if (!err) {
						...
						var message = serializedFormData.record ? app.vtranslate('JS_EVENT_UPDATED') : app.vtranslate('JS_RECORD_CREATED');
	...
	registerPopoverEvent: function (event, element, calendarView) {
		...
		var generatePopoverContentHTML = function (eventObj) {
			...
			if(eventObj.status == 'Выполнено')
			{
				setTimeout(() => {
					$("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").css({"background-color" : "black", "border-color" : "black", "color" : "white"});
					var x = $("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html();
					$("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html("<span style='color:white;background-color:#6FDE00;font-size:1.3em;border: 2.5px solid #6FDE00;'> &#128505;</span> "+ x);
				}, 10);	
			}
			else if(eventObj.status == 'Held')
			{
				setTimeout(() => {
					var x = $("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html();
					$("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html("<span style='color:white;padding-left:1.5px;padding-right:2.5px;background-color:#FF8000;font-size:1.2em;'> &#128736;</span> "+ x);
				}, 10);		
			}
			else if(eventObj.status == "Planned")
			{
				setTimeout(() => {
					var x = $("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html();
					$("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html("<span style='color:black;font-weight: bold;background-color:#FFFF00;font-size:1.2em;'>&#128276;</span> "+ x);
				}, 10);	
			}
			else if(eventObj.status == "Not Held")
			{
				setTimeout(() => {
					var x = $("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html();
					$("td.fc-event-container").find("a[href='index.php?module=Calendar&view=Detail&record="+eventObj.id+"']").children().children("span.fc-title").html("<span style='color:black;font-weight: bold;background-color:#FF0000;font-size:1.2em;'>&#9940;</span> "+ x);
				}, 10);		
			}
			return popOverHTML;
		};
	...
	getCalendarConfigs: function () {
		...
		var calenderConfigs = {
			...
			eventLimit: true,
                        ...
                        eventLimitText: app.vtranslate('LBL_MORE'),
                        //SalesPlatform.ru end
