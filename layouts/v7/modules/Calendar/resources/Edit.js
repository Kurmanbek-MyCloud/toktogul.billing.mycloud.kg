...
Vtiger_Edit_Js("Calendar_Edit_Js",{
	...
	registerBasicEvents : function(container) {
		...
		$("select[name='eventstatus']").change(()=>{
			var a = $("select[name='eventstatus'] option:selected").val();
			setTimeout(()=>{
				if(a != 'Выполнено')
				{
					$("table").find("input#Events_editView_fieldName_cf_1107").parent().parent().find("td:eq(2)").hide();
					$("table").find("input#Events_editView_fieldName_cf_1107").parent().parent().find("td:eq(3)").hide();
				}
				else
				{
					$("table").find("input#Events_editView_fieldName_cf_1107").parent().parent().find("td:eq(2)").show();
					$("table").find("input#Events_editView_fieldName_cf_1107").parent().parent().find("td:eq(3)").show();
