			...
			<ul class="dropdown-menu">
				<li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="{$LISTVIEW_ENTRY->getFullDetailViewUrl()}&app={$SELECTED_MENU_CATEGORY}">{vtranslate('LBL_DETAILS', $MODULE)}</a></li>
					{if $RECORD_ACTIONS}
						...
						<li><a data-project-name="{$LISTVIEW_ENTRY->getName()}" data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" onclick="Vtiger_List_Js.addActivity(this, '{$MODULE}');">Добавить активность</a></li>
						{/if}
					{/if}
			</ul>
		</span>
