...
<div class="col-sm-12 col-xs-12 ">
	...
	{/if}
{* <pre>
{var_dump($CURRENT_CV_MODEL->isCvEditable())}
{var_dump($CURRENT_CV_MODEL)}
</pre> *}
					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
							<span class="fieldValue">
								<span class="value" {if (is_numeric($LISTVIEW_ENTRY_VALUE) and $LISTVIEW_ENTRY_VALUE > 0 and $LISTVIEW_HEADER->get('name') == 'cf_1289')} style="color:#ca2913" {elseif (is_numeric($LISTVIEW_ENTRY_VALUE) and $LISTVIEW_ENTRY_VALUE < 0 and $LISTVIEW_HEADER->get('name') == 'cf_1289')} style="color:#489e56" {else} style="color:#000" {/if}>
