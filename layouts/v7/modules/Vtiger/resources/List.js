...
Vtiger.Class("Vtiger_List_Js", {
	...
	addActivity: function (obj, module) {
		var projectId = $(obj).data('id');
		var projectName = $(obj).data('project-name');
		$('#menubar_quickCreate_Events').trigger('click');
		setTimeout(() => {
			$('.referenceModulesList').val(module).trigger('change');
			$('[name=parent_id]').val(projectId);
			$('[name=parent_id_display]').val(projectName);
			$('input[name=parent_id_display]').attr("readonly", "readonly");
			$("a.clearReferenceSelection").removeClass("hide");
		}, 700);
		console.log("addactivity");
	},
	// registerAddActivity: function () {
	...
	//         $('#menubar_quickCreate_Events').trigger('click');
	//         setTimeout(function () {
	//             $('.referenceModulesList').val('Project').trigger('change');
	//             $('[name=parent_id]').val(projectId);
	// 			$('[name=parent_id_display]').val(projectName);
	//         }, 500);
	// 	});
	// 	console.log("registerAddActivity");
	// },


	triggerPrintPDF: function () {
		...
		if (listSelectParams) {
			...
			var href = path + '?module=Invoice&selectedIds=' + selectedIds + '&viewname=' + viewName;
			window.open(href, '_blank').focus();
		}
	},
	triggerPrintPDF_SPPayments: function () {
		...
		if (listSelectParams) {
			...
			var selectedIds = listSelectParams.selected_ids;
			var viewName = listSelectParams.viewname;
			var path = '/PrintPDF/PrintPDF_SPPayments.php';
			// window.location.href = path+'?module=Invoice&selectedIds='+selectedIds+'&viewname=' + viewName;
			var href = path + '?module=SPPayments&selectedIds=' + selectedIds + '&viewname=' + viewName;
			window.open(href, '_blank').focus();
		}
	},
	send_to_whatsapp: function () {
		...
		if (listSelectParams) {
			...
			var path = 'send_msg/sent_invoice_to_whatsapp.php';
			// window.location.href = path+'?module=Invoice&selectedIds='+selectedIds+'&viewname=' + viewName;
			var href = path + '?module=Invoice&selectedIds=' + selectedIds + '&viewname=' + viewName;
			window.open(href, '_blank').focus();
		}
	...
	getListSearchParams: function (includeStarFilters) {
		...
		console.log("listSearchParams:", listSearchParams);
