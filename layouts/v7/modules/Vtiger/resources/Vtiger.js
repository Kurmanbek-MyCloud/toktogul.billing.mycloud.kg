 ...
 * All Rights Reserved.
 *************************************************************************************/
var choiceBtn = $('td.fieldValue > select[name="cf_1444"]');
// console.log(choiceBtn.parent())
choiceBtn.change(function(){ 
    var value = $(this).val();
	var fieldNumOfFlat = $('td.fieldValue > input[name="cf_1446"]')
	var parFieldNumOfFlat = fieldNumOfFlat.parent()
	var grandParFieldNumOfFlat = parFieldNumOfFlat.parent().children()
	if(value=="Дом"){
		grandParFieldNumOfFlat.eq(2).hide()
		grandParFieldNumOfFlat.eq(3).hide()
	}else{
		grandParFieldNumOfFlat.eq(2).show()
		grandParFieldNumOfFlat.eq(3).show()
	}
});


var td = $("td#Flats_detailView_fieldValue_cf_1444 > span.value");
var td23 = $("td#Flats_detailView_fieldLabel_cf_1446");
var tr_td = td23.parent().children();
if(td.text().trim() == "Дом"){
	tr_td.eq(0).hide();
	tr_td.eq(1).hide();
}
...
Vtiger.Class('Vtiger_Index_Js', {
	...
	showNotifs: function(){
		...
		popupInstance.showPopup(params, null, function(data){
			var idList = [];
			$("#modal-notifs-menu > li").each(function(){
				var c = $(this);
				idList.push(c.data("id"));
			});
			app.request.post({data: {
				...
				$("#hd_notifs").removeClass("hd_notifs_has");
			});
		});
	},
	removeNotif: function(row) {
		var params = {
			module: "Vtiger",
		...
		app.request.post({data: params}).then(function(err, result){
			row.remove();
			if($("#modal-notifs-menu").children().length == 0) $("#modal-content-wrp").empty().append("<h3 id=\"notifs_empty\">Уведомлений нет</h3>");
		});
	},
	/**
...
//	showComposeEmailPopup : function(params, cb){
	...
	registerEvents: function() {
		...
		$("li#quickCreateModules").find("a#menubar_quickCreate_Calendar").hide();
		$("li#quickCreateModules").find("a#menubar_quickCreate_Calendar").parent().hide();
		$("button[title='Добавить Задачу']").hide();
		$("button#Calendar_listView_basicAction_LBL_ADD_TASK").hide();
		// alert();
		$("a#menubar_quickCreate_Events").on('click',()=>{
			
		setTimeout(() => {	
			$(".quickCreateContent.calendarQuickCreateContent").css({"margin-top": "1px"});
			$(".quickCreateContent.calendarQuickCreateContent div div table tbody").find("input#Events_editView_fieldName_cf_1107").parent().parent().hide();
			$(".quickCreateContent.calendarQuickCreateContent select[name='eventstatus']").change(()=>{
				var a = $(".quickCreateContent.calendarQuickCreateContent select[name='eventstatus'] option:selected").val();
					if(a != 'Выполнено')
					{
						$(".modal-body").css({"float" : "none"});
						$(".quickCreateContent.calendarQuickCreateContent div div table tbody").find("input#Events_editView_fieldName_cf_1107").parent().parent().hide();
					}
					else
					{
						$(".modal-body").css({"float" : "none"});
						$(".quickCreateContent.calendarQuickCreateContent div div table tbody").find("input#Events_editView_fieldName_cf_1107").parent().parent().show();
					}
			});
		}, 1500);
		});
					...
					$.ajax({
					    ...
					    if(data.rows){
					    	...
					    	for(var row of data.rows){
					    		html+=`<a href="index.php?module=Contacts&view=Detail&record=${row.crmid}" target="_blank"><div class="resultRow">${row.name}</div></a>`	
					    	}
							html+=`</div>`
...
//					
	...
	getPopUpParams : function(container) {
		...
		// jeng
		if (sourceField === 'cf_1341') {

			var accountId = $('input[name=cf_1339]').val();
