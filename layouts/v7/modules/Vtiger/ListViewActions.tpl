        ...
        <div class = "row">
            ...
            <div class="btn-group listViewActionsContainer" role="group" aria-label="...">
                    ...
                    <div class="btn-group listViewMassActions" role="group">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            {if $MODULE eq 'Invoice'}
                                <li class="hide">
                                    <a id="{$MODULE}_listView_massAction_LBL_PRINT_PDF" target="_blank" onclick="Vtiger_List_Js.triggerPrintPDF()">
                                        Распечатать список в PDF
                                    </a>
                                </li>
                                <li class="divider hide"></li>
                            {/if}
                            {if $MODULE eq 'Invoice'}
                                <li class="hide">
                                    <a id="{$MODULE}_listView_massAction_LBL_SEND_MSG" target="_blank" onclick="Vtiger_List_Js.send_to_whatsapp()">
                                        Отправить счет на whatsapp
                                    </a>
                                </li>
                                <li class="divider hide"></li>
                            {/if}
                            {if $MODULE eq 'SPPayments'}
                                <li class="hide">
                                    <a id="{$MODULE}_listView_massAction_LBL_PRINT_PDF" target="_blank" onclick="Vtiger_List_Js.triggerPrintPDF_SPPayments()">
                                        Распечатать платёж в PDF
                                    </a>
                                </li>
                                <li class="divider hide"></li>
                            {/if}
                ...
                <div>
                    ...
                    <span><b>Прибыль всего: </b> </span>
                        {$MODULE_MODEL->allCashSum()} ⊆
                    {/if}
                    {if $MODULE == 'Invoice'}
                        <span style = "color: red;"><b>Эсеп-билдирмелер ай сайын 8-де саат 1:00-дө түзүлөт!</b> </span><br>
                        <span style = "color: red;"><b>Туура түзүү үчүн айдын 7-күнү саат 23:00-гө чейин көрсөткүчтөрдү чогултуу керек!</b> </span>
                    {/if}
                     {if $MODULE eq 'Meters'}
                         <button type="button" class="btn btn-info" data-toggle="modal" data-target="#neprocessedCountersModal">
                            Просмотреть счетчики
                        </button>
                    {/if}

                    
                    {if $MODULE eq 'MetersData'}
{*                        <button type="button" class="btn btn-info" id="fetchElephantData">*}
{*                            Стянуть показания с Элеханта*}
{*                        </button>*}

                        <script>
                            console.log('Скрипт загружен');
                            document.addEventListener('DOMContentLoaded', function () {
                                const button = document.getElementById('fetchElephantData');
                                
                                if (button) {
                                    console.log('Кнопка найдена, добавляем обработчик');
                                    button.addEventListener('click', function (event) {
                                        console.log('Кнопка нажата');
                                        const userConfirmed = confirm('Вы уверены, что хотите стянуть показания с Элеханта?');
                                    if (userConfirmed) {
                                        window.open('https://toktogul.billing.mycloud.kg/metersDataConnector.php', '_blank');
                                    } else {
                ...
                {if $MODULE eq 'MetersData'}
                    <button type="button" class="btn btn-light" id="fetchQuant">
                        Загрузить xlsx файл с показаниями Элехант
                    </button>
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            const button = document.getElementById('fetchQuant');
                            if (button) {
                                button.addEventListener('click', function () {
                                    window.open('https://toktogul.billing.mycloud.kg/page_loader_elehant/main.php', '_blank');
                                });
                            }
                        });
                    </script>
                ...
                {if $MODULE eq 'MetersData'}
                    <button type="button" class="btn btn-light" id="fetchQuant_kg">
                        Стянуть показания с Quant.kg
                    </button>
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            const button = document.getElementById('fetchQuant_kg');    
                            if (button) {
                                button.addEventListener('click', function () {
                                    window.open('https://toktogul.billing.mycloud.kg/connector_quant/index.php', '_blank');
                                });
                            }
                        });
                    </script>
...
<!-- Модальное окно -->
<div class="modal fade" id="neprocessedCountersModal" tabindex="-1" aria-labelledby="neprocessedCountersModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="neprocessedCountersModalLabel">Список счетчиков</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Выводим количество счетчиков -->
        <p>Всего счетчиков: <span id="totalCounters"></span></p>
        <p>Обработано счетчиков: <span id="processedCounters"></span></p>
        <p>Не обработано счетчиков: <span id="unprocessedCounters"></span></p>
        
        <!-- Список необработанных счетчиков -->
        <h5>Список необработанных счетчиков:</h5>
        <ul id="unprocessedMetersList"></ul>
      </div>
    </div>
  </div>
</div>
...
$(document).ready(function() {
    // Делаем AJAX-запрос к файлу getDifferentMeters.php
    $.ajax({
        url: 'getDifferentMeters.php',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response) {
                // Заполняем данные
                $('#totalCounters').text(response["Всего счетчиков"]);
                $('#processedCounters').text(response["Обработано счетчиков"]);
                $('#unprocessedCounters').text(response["Не обработано счетчиков"]);
                
                // Отображаем список необработанных счетчиков
                let metersList = response["Список счетчиков которые есть в биллинге но нету на сайте Элехант"];
                $('#unprocessedMetersList').empty();  // Очищаем список перед добавлением новых данных
                for (let key in metersList) {
                    if (metersList.hasOwnProperty(key)) {
                        $('#unprocessedMetersList').append('<li>' + metersList[key] + '</li>');
                    }
                }
            } else {
