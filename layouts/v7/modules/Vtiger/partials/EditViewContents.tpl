			...
			{if $BLOCK_FIELDS|@count gt 0}
				<div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}">
								...
								{if $FIELD_MODEL->isEditable() eq true}
									...
									{if $FIELD_MODEL->get('uitype') neq '83'}
										<td class="fieldValue" {if $FIELD_MODEL->getFieldDataType() eq 'boolean'} style="width:25%" {/if} {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
											{* {if $FIELD_MODEL->get('label') == "Meter"} <input type="text" id="Meters_editView_fieldName_meter" name="meter" data-fieldname="meter" class="inputElement nameField input-number" value data-rule-required="true" aria-required="true" oninput="removeLeadingZeros(this)"> 



											{else} {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE)} {/if} *}
					...
					function removeLeadingZeros(input) {
						...
						removeLeadingZeros(document.querySelector(".input-number"))
				</script> *}
				</div>
			{/if}
		{/foreach}
