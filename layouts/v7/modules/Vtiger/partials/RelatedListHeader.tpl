...
{strip}
									{* <pre>
									{$RELATED_MODULE_NAME}<br>
									1<br>
									{$RELATED_LINK->getUrl()}<br>
									2<br>
									{$RELATED_LINK_URL}<br>
									3<br>
									{var_dump($PARENT_RECORD->get('cf_1235'))}
									</pre> *}
					...
					<div class="btn-group">
						...
						{else}
							...
							{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}
								{assign var="RELATED_LINK_URL" value=$RELATED_LINK->getUrl()}
								...
								<button type="button" module="{$RELATED_MODULE_NAME}" class="btn btn-default
									...
									{if ($RELATED_LINK->isPageLoadLink())}
										...
										data-url="{$RELATED_LINK_URL}{if $SELECTED_MENU_CATEGORY}&app={$SELECTED_MENU_CATEGORY}{/if}"
									{/if}
									>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>&nbsp;{/if}&nbsp;{$RELATED_LINK->getLabel()}</button>
									{if $RELATED_MODULE_NAME == 'MetersData' && $MODULE == "Meters"}
									<a href="add_meter_elehant.php/?url={$RELATED_LINK_URL}">
										<button type="button" module="{$RELATED_MODULE_NAME}" class="btn btn-default
									{if $IS_SELECT_BUTTON eq true} addButton" name="addButton{/if}"
									{if $IS_SELECT_BUTTON eq true} data-moduleName="{$RELATED_LINK->get('_module')->get('name')}" {/if}
									>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>&nbsp;{/if}&nbsp;Добавить показания с Элехант</button></a>
									{/if}
							{/if}
						{/if}
					</div>
