    ...
    <span class="more dropdown action">
        ...
        <ul class="dropdown-menu">
			...
			{if $RECORD_ACTIONS}
				...
				{/if}
			{/if}
            {if $MODULE eq 'Leads' || $MODULE eq 'HelpDesk' || $MODULE eq 'SalesOrder' || $MODULE eq 'Contacts' ||$MODULE eq 'Potentials' || $MODULE eq 'Accounts'}<li><a data-project-name="{$LISTVIEW_ENTRY->getName()}" data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" onclick="Vtiger_List_Js.addActivity(this, '{$MODULE}');">Добавить активность</a></li>{/if}
        </ul>
    </span>
