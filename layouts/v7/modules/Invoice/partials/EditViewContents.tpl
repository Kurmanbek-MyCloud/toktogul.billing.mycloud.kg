...
{* <script>
    $(document).ready(function(){
        ...
        function readElem(){
            const elements=document.querySelectorAll(".qty.smallInputBox.inputElement");
            if(elements.length>0){
                  for(let i=0; i<elements.length;i++){
                    const key=elements[i];
                    const parent=$(key).parents().closest("tr").find(".listPrice");
                    const service=$(key).parents().closest("tr").find(".productName").val();
                    console.log(service);
                    const oldValue=$(key).parents().closest("tr").find(".old_price").val();
                    console.log(oldValue);
                    if (service === 'Вода - Уличный'){
        ...
        readElem();
        $("#addService").click('.lineItemRow',function(e){
            e.preventDefault();
            readElem();
