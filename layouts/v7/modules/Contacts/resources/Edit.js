...
Vtiger_Edit_Js("Contacts_Edit_Js",{},{
	...
	registerOnChangeFunctions: function () {
		$('input[name=birthday]').on('change', () => {
			let bd = $('input[name=birthday]').val();
			// alert(this.val());
			// console.log(`bd: ${bd}`);
			// console.log(bd.split('-'));
			let d = bd.split('-')
			let bddate = new Date(`${d[2]}-${d[1]}-${d[0]}`);
			console.log(`bddate ${bddate}`);
			var now = new Date(); //Текущя дата
			var today = new Date(now.getFullYear(), now.getMonth(), now.getDate()); //Текущя дата без времени
			var dob = new Date(bddate.getFullYear(), bddate.getMonth(), bddate.getDate()); //Дата рождения
			...
			let fullage = `${age} ${postfix}`;
			console.log(`fullage ${fullage}`);
			// alert(age)
			// document.write (`Возраст: ${age}`);
			$('input[name=cf_1408]').val(fullage);
		});
	},
	...
	registerBasicEvents: function (container) {
		...
		this.registerOnChangeFunctions();
	}
})