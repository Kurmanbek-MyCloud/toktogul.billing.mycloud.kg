...
function SPPayments_IndexView_Component($scope, $api, $webapp, $modal, sharedModalService, $translatePartialLoader, $rootScope, $http, $translate) {

	if ($translatePartialLoader !== undefined) {
		$translatePartialLoader.addPart('home');
		$translatePartialLoader.addPart('SPPayments');
	}

	var availableModules = JSON.parse(localStorage.getItem('modules'));
	var currentModule = 'SPPayments';
	//set creatable true
	...
	$scope.$on('serviceSPPayments', function () {
		window.location.href = "index.php?module=SPPayments&status=Open";
	});

	$scope.$on('editRecordModalSPPayments.Template', function () {
		$modal.open({
			templateUrl: 'editRecordModalSPPayments.template',
			controller: SPPayments_EditView_Component,
			backdrop: 'static',
			size: 'lg',
			keyboard: 'false',
			resolve: {
	...
	if (loadStatus !== undefined && loadStatus !== '') {
		...
		var stateObject = {};
		var title = "Portal";
		var newUrl = "index.php?module=SPPayments";
...
function SPPayments_DetailView_Component($scope, $api, $webapp, $translatePartialLoader, sharedModalService, $modal) {
	$scope.module = 'SPPayments';
	if ($translatePartialLoader !== undefined) {
		$translatePartialLoader.addPart('home');
		$translatePartialLoader.addPart('SPPayments');
	...
	$scope.$watch('SPPaymentsStatus', function (nvalue, ovalue) {
			...
			recordId: $scope.id
				...
				.success(function (savedRecord) {
					if (savedRecord.record !== undefined) {
						...
						$scope.SPPaymentsStatus = recordStatus;
						var statusField = $scope.edits[ 'ticketstatus' ];
						$scope.record[ statusField ] = $scope.SPPaymentsCloseLabel;
						$scope.closeButtonDisabled = false;
						$webapp.busy(false)
...
function SPPayments_EditView_Component($scope, $modalInstance, record, api, webapp, module, $timeout, $translatePartialLoader, language, $filter, $http, editStatus) {
		...
		api.post(module + '/SaveRecord', params)
				.success(function (savedRecord) {
					...
					if (savedRecord.record !== undefined) {
						...
						window.location.href = 'index.php?module=SPPayments&view=Detail&id=' + savedRecord.record.id;
					}
					if (savedRecord.record === undefined) {
