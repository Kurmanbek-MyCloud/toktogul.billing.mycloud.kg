...
function Home_Component($scope, $api, $webapp, $modal, sharedModalService, $translatePartialLoader) {
	...
	$scope.fetchCustomData = function () {
        $api.get('Portal/FetchCustomData').success(function (result) {
            $scope.customDataFetched = true;
            $scope.personal_account = result['personal_account'];
            $scope.balance = result['balance'];
            $scope.total_debt = result['total_debt'];
        });
    }
	...
	$scope.fetchShortcuts = function () {
		...
		$api.get('Portal/FetchShortcuts').success(function (result) {
			...
			if (result.shortcut) {
                ...
                if (availableModules['MetersData'] !== undefined && result.shortcut.MetersData !== undefined) {
                    var uiLabel = availableModules['MetersData'].uiLabel;
                    result.shortcut[uiLabel] = result.shortcut.MetersData;

                    if (uiLabel !== 'MetersData')
                        delete result.shortcut['MetersData'];
                }
	...
	$scope.openShortcut = function (module, action) {
		...
		var actionConfig = {
			...
			'LBL_CREATE_READINGS': 'MetersData'
		};
		var serviceConfig = {
	...
	angular.extend(this, new MetersData_IndexView_Component($scope, $api, $webapp, $modal, sharedModalService, $translatePartialLoader));
