...
$server->register(
	'get_sppayments_detail',
	array('id'=>'xsd:string','block'=>'xsd:string','payer'=>'xsd:string','sessionid'=>'xsd:string'),
	array('return'=>'tns:field_details_array'),
	$NAMESPACE);
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	else if($module == 'SPPayments')
	{
		$query ="select distinct sp_payments.*,vtiger_crmentity.smownerid,
		case when sp_payments.payer !=0 then sp_payments.payer else sp_payments.accountid end as entityid,
		case when sp_payments.payer !=0 then 'Contacts' else 'Accounts' end as setype
		from sp_payments
		left join vtiger_crmentity on vtiger_crmentity.crmid=sp_payments.invoiceid
		where vtiger_crmentity.deleted=0 and (payer in (". generateQuestionMarks($entity_ids_list) .") or payer in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
        // SalesPlatform.ru begin
...
function get_sppayments_detail($id,$module,$customerid,$sessionid)
{
	require_once('include/utils/UserInfoUtil.php');
	require_once('include/utils/utils.php');
	...
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select sp_payments.*,vtiger_crmentity.* ,
		sp_paymentscf.* from sp_payments
		INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = sp_payments.payid
		INNER JOIN sp_paymentscf ON sp_payments.payid = sp_paymentscf.payid
		where sp_payments.payid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($fieldname == 'subject' && $fieldvalue !='')
		{
			$fieldid = $adb->query_result($res,0,'payid');
			//$fieldlabel = "(Download PDF)  ".$fieldlabel;
			$fieldvalue = '<a href="index.php?downloadfile=true&module=SPPayments&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
		}
		if( $fieldname == 'salesorderid' || $fieldname == 'contactid' || $fieldname == 'accountid' || $fieldname == 'potentialid')
		{
			$crmid = $fieldvalue;
...
function get_product_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	/*$checkPayments = checkModuleActive('SPPayments');
	if($checkPayments == true){
		$query[] = "select distinct vtiger_products.*,
			case when vtiger_invoice.contactid !=0 then vtiger_invoice.contactid else vtiger_invoice.accountid end as entityid,
			case when vtiger_invoice.contactid !=0 then 'Contacts' else 'Accounts' end as setype
			from vtiger_invoice
...
function check_permission($customerid, $module, $entityid) {
	...
	switch($module) {
		...
		case 'Invoice'	:	$query = "SELECT vtiger_invoice.invoiceid
							...
							break;	
		case 'SPPayments'	:	$query = "SELECT sp_payments.payid
								FROM sp_payments
								INNER JOIN vtiger_crmentity
								ON sp_payments.payid=vtiger_crmentity.crmid
								WHERE vtiger_crmentity.deleted=0
									AND (sp_payments.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or sp_payments.payer IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND sp_payments.payid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
