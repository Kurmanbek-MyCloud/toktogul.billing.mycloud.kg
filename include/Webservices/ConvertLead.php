...
function vtws_convertlead($entityvalues, $user, $so = null) {
	...
	$availableModules = array('Accounts', 'Contacts', 'Potentials', 'SalesOrder');
	...
	if($leadId[1]) {
		...
		if($so) {
			//Эта часть нужна для преобразования лида в заказ 
			//Просто копируем и вставляем в код)
			if(isset($contactId)) $so['contact_id'] = $contactId;
			if(isset($accountId)) $so['account_id'] = $accountId;
			$entityData->so = $so;
		}
