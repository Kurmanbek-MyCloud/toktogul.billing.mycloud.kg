...
class Vtiger_Save_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			if($request->has("mentions")) $this->parseMentions($request);
	...
	private function parseMentions(Vtiger_Request $request) {
		...
		if(!empty($mentions)) {
			...
			foreach($mentions as $id => $val) {
				if($id == $assignedBy) continue;
				if($qStr != "") $qStr .= ",";
				$qStr .= "('$url','$module',$id,$assignedBy,$record)";
			}
			file_put_contents("/var/www/html/data.txt", $qStr);
			$adb->pquery("
				INSERT INTO vtiger_notifications(link,module,assigned_to,added_by,recordid) VALUES $qStr
			");
		}
	}
	...
	public function saveRecord($request) {
		...
		if($request->get('relationOperation')) {
			...
			// если функция связи не равна get_dependents_list, то добавляем связь (таблица vtiger_crmentityrel)
			$relationModel = Vtiger_Relation_Model::getInstance($parentModuleModel, $relatedModule);
			$relFuncName = $relationModel->get('name');
			if ($relFuncName != 'get_dependents_list')
            {
