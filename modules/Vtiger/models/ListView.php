	...
	public function getListViewEntries($pagingModel)
	{
		...
		if (!empty($orderBy) && $orderByFieldModel) {
			if ($orderBy == 'roleid' && $moduleName == 'Users') {
				...
				if ($orderBy == 'flat' && $moduleName == 'Flats') {

					// $listQuery .= ' ORDER BY ' . $queryGenerator->getOrderByColumn($orderBy) . ' ' . $sortOrder;
					$listQuery = "SELECT
						vtiger_flatscf.cf_1450,
						vtiger_flatscf.cf_1448,
						vtiger_flats.flat,
						vtiger_flatscf.cf_1446,
						vtiger_flatscf.cf_1289,
						vtiger_flatscf.cf_1235,
						vtiger_flatscf.cf_1420,
						vtiger_contactscfcf_1235.cf_1468 AS cf_1235cf_1468,
						vtiger_contactscfcf_1235.contactid AS cf_1235cf_1468_id,
						vtiger_flats.flatsid
					FROM
						vtiger_flats
					INNER JOIN
						vtiger_crmentity ON vtiger_flats.flatsid = vtiger_crmentity.crmid
					INNER JOIN
						vtiger_flatscf ON vtiger_flats.flatsid = vtiger_flatscf.flatsid
					LEFT JOIN
						vtiger_contactscf AS vtiger_contactscfcf_1235 ON vtiger_contactscfcf_1235.contactid = vtiger_flatscf.cf_1235
					WHERE
						vtiger_crmentity.deleted = 0
						AND vtiger_flats.flatsid > 0
					ORDER BY
						CAST(SUBSTRING_INDEX(vtiger_flats.flat, ' ', 1) AS UNSIGNED) $sortOrder";
				} else if ($orderBy == '(cf_1265 ; (Flats) flat)' && $moduleName == 'Invoice') {
					$listQuery .= ' ORDER BY ' . $queryGenerator->getOrderByColumn($orderBy) . ' ' . $sortOrder;
					// var_dump($listQuery);
					// exit();
					$listQuery = "SELECT vtiger_invoice.subject, 
								vtiger_invoice.contactid, 
								vtiger_invoice.invoicestatus, 
								vtiger_invoice.total, 
								vtiger_flatscfcf_1265.cf_1448 AS cf_1265cf_1448, 
								vtiger_flatscfcf_1265.flatsid AS cf_1265cf_1448_id, 
								vtiger_flatscf_1265.flat AS cf_1265flat, 
								vtiger_flatscf_1265.flatsid AS cf_1265flat_id, 
								vtiger_flatscfcf_1265.cf_1446 AS cf_1265cf_1446, 
								vtiger_flatscfcf_1265.flatsid AS cf_1265cf_1446_id, 
								...
								LEFT JOIN vtiger_flatscf AS vtiger_flatscfcf_1265 ON vtiger_flatscfcf_1265.flatsid = vtiger_invoicecf.cf_1265 
								LEFT JOIN vtiger_flats AS vtiger_flatscf_1265 ON vtiger_flatscf_1265.flatsid = vtiger_invoicecf.cf_1265 
								WHERE vtiger_crmentity.deleted=0 
								AND vtiger_invoice.invoiceid > 0 
								ORDER BY CAST(SUBSTRING_INDEX(vtiger_flatscf_1265.flat, ' ', 1) AS UNSIGNED) $sortOrder";
				} else {
					// var_dump($orderBy);
					// var_dump($moduleName);
					// exit();
