...
class Mobile_HTML_Viewer extends Vtiger_Viewer{
	...
	function viewController() {
		...
		$this->_smarty->assign("TEMPLATE_WEBPATH", "layouts/".Vtiger_Viewer::getLayoutName()."/modules/Mobile/".$this->getLayout());

		return $this->_smarty;
	}

	function process($module, $view) {
	    if ($this->newDesign && $view !== 'Login') {
            $view = 'Main';
        }
	...
	function _getBaseTemplatePath() {
		$smarty = $this->viewController();
		return $smarty->getTemplateDir(0) . DIRECTORY_SEPARATOR . "modules/Mobile/". $this->getLayout();
	}

    /**
     * @return $this
     */
    public function withNewDesign()
    {
        $this->newDesign = true;
    ...
    {
        return $this->newDesign ? Mobile::config('Mobile.Layout') : Mobile::config('Default.Layout');
    }
