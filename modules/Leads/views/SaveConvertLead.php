...
class Leads_SaveConvertLead_View extends Vtiger_View_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		$entityValues['transferRelatedRecordsTo'] = $request->get('transferModule') == "SalesOrder" ? "Contacts" : $request->get("transferModule");
		...
		//Эта часть отвечает за преобразование из лида в заказ
		//Здесь необоходимо назначить какие данные нужно передать в заказ при создании
		$so = null;
		if(in_array("SalesOrder", $modules)) $so = array(
			"productid" => $request->get("productid"),
			"subject" => $request->get("subject"),
			"listprice" => $request->get("listprice"),
			"sostatus" => $request->get("sostatus")
		);
		try {
			//Затем необоходимо передать переменную $so третьим аргументом
			$result = vtws_convertlead($entityValues, $currentUser, $so);
		} catch(Exception $e) {
