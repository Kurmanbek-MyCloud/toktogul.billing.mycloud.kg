...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	function getConvertToSalesLeadFields() {
		global $adb;
		$res = array();
		$soInstance = Vtiger_Module_Model::getInstance("SalesOrder");
		$qStr = "
			SELECT fieldid, defaultvalue, columnname, typeofdata FROM vtiger_field WHERE tablename = 'vtiger_salesorder' UNION
			SELECT vf.fieldid as fieldid,vf.defaultvalue as defaultvalue,vf.columnname as columnname, vf.typeofdata as typeofdata FROM vtiger_field as vf JOIN
			vtiger_relatedlists as vr ON ((vr.tabid = 14 AND vr.related_tabid = 22) OR (vr.tabid = 22 AND vr.related_tabid = 14))
			AND vf.fieldid = vr.relationfieldid
		";
		$fieldsQ = $adb->pquery($qStr,array());
		$nums = $adb->num_rows($fieldsQ);
		$fields = $soInstance->getFieldsById();
		for($i = 0; $i < $nums; ++$i) {
			$row = $adb->query_result_rowdata($fieldsQ, $i);
			$fid = $row['fieldid'];
			if(!empty($row['defaultvalue'])) {
				$moduleField = $soInstance->getField($row['columnname']);
				if(is_object($moduleField)) $fields[$fid]->set("fieldvalue", $moduleField->getDefaultFieldValue());
			}
			$split = explode("~", $row['typeofdata']);
			if($split[1] != "M") continue;
			if($fid && isset($fields[$fid])) $res[] = $fields[$fid];
		}
		$prodField = $soInstance->getField("listprice");
		$prodField->set('fieldvalue', $prodField->getDefaultFieldValue());
		$res[] = $prodField;
		return $res;
	}
