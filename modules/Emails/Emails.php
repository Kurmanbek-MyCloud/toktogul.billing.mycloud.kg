		...
		if ($_REQUEST['module'] == "Emails" && $_REQUEST['smodule'] != 'webmails' && (!$this->plugin_save)) {
			...
			if ($_REQUEST['module'] != 'Emails' && $_REQUEST['module'] != 'Webmails') {
				...
				for ($i = 0; $i < (count($myids) - 1); $i++) {
					...
					$sourceModuleRecordId = $_REQUEST['source_module_record_id'];

					if ($sourceModuleRecordId != $mycrmid)
					{
						$adb->query("insert into vtiger_seactivityrel values ($sourceModuleRecordId, $actid)");
					}
				}
			}
		} else {
