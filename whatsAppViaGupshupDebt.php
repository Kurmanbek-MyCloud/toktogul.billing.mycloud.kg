...
global $adb;
$logger = new CustomLogger('whatsAppViaGupshupDebt.log');
// exit();
...
// exit();
##############################################################################################
################################# Рассыллка для должников ####################################
##############################################################################################


$res = $adb->pquery("SELECT M.meter, CONCAT('996',right(CD.mobile,9)) AS mobile, CD.lastname,
                        (
                          ...
                          -
                          (SELECT IFNULL( (select round(SUM(amount),2) as summ FROM sp_payments as SP
                    ...
                    INNER JOIN vtiger_flatscf FCF2 ON FCF2.cf_1235 = CD.contactid
                    INNER JOIN vtiger_meterscf AS MCF ON MCF.cf_1319 = FCF2.flatsid
                    INNER JOIN vtiger_meters AS M ON MCF.metersid = M.metersid
						  WHERE CRM.deleted = 0
                    AND 	(
                          (SELECT IFNULL((SELECT round(sum(total),2) AS summ FROM vtiger_invoice AS I
                                        INNER JOIN vtiger_crmentity AS CE ON I.invoiceid = CE.crmid
...
$logger->log("Количество должников: " . $adb->num_rows($res));
for ($i=0; $i < $adb->num_rows($res); $i++) { 
  // for ($i=0; $i < 1; $i++) { 
    $meter = $adb->query_result($res,$i,'meter');
    ...
    curl_setopt_array($curl, array(
          ...
          '&source=996999901767'.
          '&template={"id": "86125721-782e-487d-b8ac-b8dc2d727654","params": ["' . 
          $lastname . '","*' . 
          $debt .'*","'.$meter.'"]}',
          CURLOPT_HTTPHEADER => array(
            'apikey: 9lf17q29xkqsu2o7nlbavclan4s2itgq',
      ...
      $response = curl_exec($curl);
      $logger->log("Sending.. #$i номер: $mobile ФИО: $lastname Задолженность: $debt response: $response");
